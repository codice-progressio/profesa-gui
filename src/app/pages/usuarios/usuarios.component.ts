...
export class UsuariosComponent implements OnInit {
  ...
  constructor(
    ...
    //Cargamos los roles.
    this._usuarioServivce.cargarPermisos().subscribe(roles => {
      this.permisos = roles
      this.permisos = this.permisos.filter(x => {
        x = x.toString()
        return x !== 'SUPER_ADMIN' && !x.includes(',')
      })

      this.permisos.sort((a, b) => (a > b ? 1 : -1))
    })
  }
  ...
  cargarUsuarios() {
    ...
    this._usuarioServivce.cargarUsuarios(this.desde).subscribe((resp: any) => {
      this.usuarios = <Usuario[]>(
        resp.usuarios.filter((x: Usuario) => x.nombre !== 'SUPER-ADMIN')
      )
  ...
  borrarUsuario(usuario: Usuario) {
    ...
    swal({
      ...
      if (borrar.value) {
        this._usuarioServivce.delete(usuario._id).subscribe(() => {
          this.cargarUsuarios()
        })
      }
    })
  ...
  guardarUsuario(usuario: Usuario) {
    const a: any = () => {
      if (usuario._id) {
        this._usuarioServivce.update(usuario).subscribe(() => {
          this.cargarUsuarios()
  ...
  guardarNuevoUsuario(usuario: Usuario) {
    this._usuarioServivce.save(usuario).subscribe(() => {
      this.cargarUsuarios()
      this.usuarioEditando = null
    })
  }
  ...
  cambiarVisibilidad(e) {
    ...
    if (this.usuarioEditando.role.includes(role)) {
      this.usuarioEditando.role = this.usuarioEditando.role.filter(
        x => x !== role
      )
    } else {
