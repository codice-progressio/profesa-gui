...
export class AlmacenDescripcionComponent implements OnInit {
  ...
  cargarAlmacenes() {
    this._almacenDescripcionServices
      .todo()
      .subscribe((almacenes) => (this.almacenesDescripcion = almacenes))
  }
  ...
  eliminarAlmacen(ad: AlmacenDescripcion) {
    let msj = `Si eliminas el almacen ${
      ad.nombre
    } tambien se borraran los articulos que esten relacionados a el. Aun asi quieres continuar?`

    let msj2 = `Que pena, pero esto es muy serio.  Si eliminas ${
      ad.nombre
    } no podras recuperar esta informacion y se afectara el costo del inventario, las estadisticas de uso, etc, etc, etc. Piensalo bien antes de continuar.`
