...
export class FoliosComponent
  ...
  constructor(
    ...
    this.mensajeDeEliminacion =
      "Si eliminas este elemento tendras que volver a registrar los datos."
  }
   ...
   * @memberof FoliosComponent
   */
  cargarFoliosMandadosAproducir(entregadosAProduccion: boolean) {
    this.elementos = []
  ...
  aplicarFiltros(componente: GrupoDeFiltroComponent) {
    ...
    this._folioNewService
      ...
      .setVendedor(this._usuarioService.usuario._id)
      .setPedido(
        this.componenteFiltrador.pedido ? this.componenteFiltrador.pedido : null
      )
      ...
      .setFechaFinalizacionFolioDesdeEl(
        componente.fechaFinalizacionFolioDesdeEl
          ? new Date(componente.fechaFinalizacionFolioDesdeEl).toISOString()
          : null
      )
      .setFechaFinalizacionFolioHasta(
        componente.fechaFinalizacionFolioHasta
          ? new Date(componente.fechaFinalizacionFolioHasta).toISOString()
          : null
      )
  ...
  filtrosMostrarParaFolios(enlistarComoPedidos) {
    ...
    this.componenteFiltrador.seleccionarCamposVisibles.setEntregarAProduccion(
      false
    )
  }
  ...
  enviarAProduccion(id: string) {
    ...
    this._msjService.confirmarAccion(mensajeDeConfirmacion, () => {
      this._folioNewService.iniciarProduccion(id).subscribe(() => {
        this.cargarFoliosMandadosAproducir(
          this.cargarFoliosEntregadosAProduccion
        )
      }),
